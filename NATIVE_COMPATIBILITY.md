...
| Imports | PASS | `tests/test_phase5.py` + `test_phase10_linux.py` | IMPORT_PACKAGE_V1 + MODULE_METADATA_V1 PASS 2026-09-09 (Win 8 + Linux 7): paquetes con `__init__.piton`, `pkg.fn()` via module-attr, `desde pkg importar fn`, submódulos `desde pkg.sub importar fn`, `__name__`/`__package__`/`__file__` y `sys.modules["x"]` byte-idénticos vs CPython. `importar pkg.sub`, ciclos, star, relative y estado de módulo → `IMPORT_RELATIVE_V1` |
