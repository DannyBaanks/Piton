...
| Imports | PASS | `tests/test_phase5.py` + `test_phase10_linux.py` | IMPORT_PACKAGE_V1 + MODULE_METADATA_V1 + IMPORT_RELATIVE_V1 PASS 2026-09-09 (Win 8 + Linux 6): paquetes con `__init__.piton`, `pkg.fn()` via module-attr, `desde pkg importar fn`, submódulos `desde pkg.sub importar fn`, `__name__`/`__package__`/`__file__`, `sys.modules["x"]` byte-idénticos vs CPython; más `importar pkg.sub` (top bound; `como P` → módulo más profundo), relative `desde . importar x`/`desde .mod importar f` y cadenas `pkg.sub.fn()`. `desde ..` >1 nivel, ciclos, star → `IMPORT_STAR_V1`/`IMPORT_CYCLIC_V1` |
