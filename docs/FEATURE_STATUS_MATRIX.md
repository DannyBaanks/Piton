...
| imports | PASS | PASS | PASS | `test_phase5.py` + `test_phase10_linux.py` | IMPORT_PACKAGE_V1 + MODULE_METADATA_V1 + IMPORT_RELATIVE_V1 + IMPORT_STAR_V1 PASS 2026-09-09: paquetes con `__init__.piton`, `pkg.fn()` via module-attr, `desde pkg importar fn`, submódulos `desde pkg.sub importar fn`, `__name__`/`__package__`/`__file__` y `sys.modules["x"].__name__/__package__`; `importar pkg.sub` (top bound; `como P` liga el módulo más profundo), `desde . importar x` / `desde .mod importar f` en `__init__`, cadenas `pkg.sub.fn()`, sys.modules de intermedios y `desde pkg importar *` (liga las funciones públicas; `_privada` excluida) — Win+Linux byte-idénticos vs CPython (Win 14 + Linux 6). Sigue: >1 nivel (`desde ..`), ciclos, cache (`IMPORT_CYCLIC_V1`) |
