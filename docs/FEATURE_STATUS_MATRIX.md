...
| imports | PASS | PASS | PASS | `test_phase5.py` + `test_phase10_linux.py` | IMPORT_PACKAGE_V1 + MODULE_METADATA_V1 PASS 2026-09-09: paquetes con `__init__.piton`, `pkg.fn()` via module-attr, `desde pkg importar fn`, submódulos `desde pkg.sub importar fn`, `__name__`/`__package__`/`__file__` y `sys.modules["x"].__name__/__package__` Win+Linux byte-idénticos vs CPython (Win 8 tests, Linux 7 tests). Sigue: `importar pkg.sub`, ciclos, star, relative, cache (`IMPORT_RELATIVE_V1`/`IMPORT_STAR_V1`/`IMPORT_CYCLIC_V1`) |
