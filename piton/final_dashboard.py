...
FEATURES = (
    ...
    Feature("generators", "PARTIAL", "tests/test_phase5.py + tests/test_phase10_linux.py", "GENERATOR_FRAME_V1 (real suspended frames, heap state machine, params, preserved locals, StopIteration on exhaustion, independent instances) + GENERATOR_SEND_V1 (send/enviar, just-started TypeError) + GENERATOR_THROW_V1 (throw/arrojar: marks finished + raises at caller's handler, no internal catch since yield-in-try fail-closed) + GENERATOR_CLOSE_V1 (close/cerrar: marks finished, idempotent) — Win+Linux; yield from (producir desde) fail-closed, PEP479 waits on generator return value, defaults/closures/métodos generadores fail-closed"),
