...
FEATURES = (
    ...
    Feature("imports", "PASS", "tests/test_phase5.py + tests/test_phase10_linux.py", "IMPORT_PACKAGE_V1 + MODULE_METADATA_V1 + IMPORT_RELATIVE_V1 + IMPORT_STAR_V1: paquetes con __init__.piton, pkg.fn(), desde pkg importar fn, submódulos desde pkg.sub importar fn + __name__/__package__/__file__/sys.modules; dotted importar pkg.sub (top bound, o modulo mas profundo con 'como P') + relative desde . importar x / desde .mod importar f en __init__ + cadenas pkg.sub.fn() + star desde pkg importar * (solo funciones publicas, excluye _privada) Win+Linux (desde .. a 2 niveles, ciclos, valores de atributo de modulo open -> IMPORT_CYCLIC)"),
...
FINAL_GATES = (
    ...
    Gate("NATIVE_IMPORT_SYSTEM", "PASS", "importar + desde X importar Y, dotted importar pkg.sub, relative desde . importar x, star desde pkg importar *, math.sqrt/asyncio.run builtins, fail-closed; >1-level relative and cycles remain open"),
