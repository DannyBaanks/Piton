...
FEATURES = (
    ...
    Feature("statements", "PARTIAL", "tests/test_phase5.py + tests/test_phase10_linux.py", "ITER_PROTOCOL_V1: iter/next builtins + __iter__/__next__ on custom classes (MRO dispatch) + enumerate/reversed/zip/map/filter (C runtime helpers) + for-else + StopIteration; _lower_for gap: for-loops dispatch only list/tuple/generator-call (dicts/sets/custom __iter__ raise MIRLoweringError) — code exists but native build environment broken on this machine (SIGSEGV)"),
    ...
    Feature("closures", "PASS", "tests/test_phase5.py + tests/test_phase10_linux.py", "CLOSURES_COMPLETE_V1 + initial frame ABI: closure-lifted functions use long* frame, dynamic captures, piton_closure_call_frame/piton_frame_call dispatch, 5 captures + 5 args; variadic closure migration and frame lifetime/GC remain open; Win 10 + Linux 10 tests"),
    ...
    Feature("generators", "PASS", "tests/test_phase5.py + tests/test_phase10_linux.py", "GENERATOR_FRAME_V1 + GENERATOR_SEND_V1 + GENERATOR_THROW_V1 + GENERATOR_CLOSE_V1 + YIELD_FROM_V1 + GENERATOREXIT_V1 — real suspended frames, heap state machine, send/enviar, throw/arrojar (caller handler), close/cerrar (GeneratorExit), yield from (producir desde) delegation loop (iterables + sub-generators), return with value (StopIteration), Win+Linux differential tests"),
...
FINAL_GATES = (
    ...
    Gate("NATIVE_ASYNC", "PASS", "async def + await + asyncio.run differential-tested, coroutine escape detection; TASK_SCHEDULER_V1 covers create_task/gather/sleep(0)/cancel; async with and real timers remain open"),
    ...
    Gate("YIELD_FROM_V1", "PASS", "yield from (producir desde) delegation loop: iterates the sub-iterator via iter_new/iter_next and yields each value to the caller via gen_yield; works for iterables (list/tuple literals) and sub-generators; basic PEP 380 delegation without send/throw forwarding or return value capture (V1 limitation); Win+Linux differential tests"),
    Gate("GENERATOREXIT_V1", "PASS", "generator close raises GeneratorExit inside the generator via piton_gen_close; if caught and generator yields again, RuntimeError; if not caught, generator is marked finished; piton_gen_close added to both runtimes (Win native_runtime.c / Linux freestanding C); Win+Linux tests"),
    ...
    Gate("NATIVE_STDLIB_DECLARED_SCOPE", "PASS", "native abs, min, max, sum, type, len, print, math.sqrt demonstrated on both backends"),
    Gate("STDLIB_TIER1_V1", "PARTIAL", "Linux differential corpus covers math floor/ceil/sin/cos/log, sys.exit/argv length, and os.name; Windows execution is pending, so the Tier 1 cross-platform claim remains partial"),
